...
# ── NEW: Memory Bank ──
parser.add_argument('--a-nums', default=60, type=int,
                    help='Anomaly memory slots')
parser.add_argument('--n-nums', default=60, type=int,
                    help='Normal memory slots')
...
# Scheduler: drop tại epoch 6 và 9, rate=0.3 (fix từ run trước)
parser.add_argument('--scheduler-rate',       default=0.3,    type=float)
parser.add_argument('--scheduler-milestones', default=[6, 9], type=int, nargs='+')

# ── Paths ──
parser.add_argument('--model-path',      default='model/model_ucf_intern.pth')
parser.add_argument('--checkpoint-path', default='model/checkpoint_intern.pth')
...
# ── HIVAU (frame-level annotation) — giữ lại nhưng không dùng trong loss ──
# focal_loss đã bị bỏ vì GT bị misalign sau uniform_extract
